...
using Strs, ShortStrings, LegacyStrings
...
# Return a string that fits within maxlen bytes
function trunc_str(rng, maxlen, siz)
    str = randstring(rng, siz)
    sz = sizeof(str) - maxlen + 1
    i = 1
    while (sz > i)
        i = nextind(str, i)
    end
...
const asc_rng = [ch for ch in ' ':'\u7f' if isprint(ch)]
const lat_rng = [ch for ch in ' ':'\uff' if isprint(ch)]
const ucs_rng = [ch for ch in [' ':'\ud7ff';'\ue000':'\uffff'] if isprint(ch)]
const uni_rng = [ch for ch in [' ':'\ud7ff';'\ue000':'\uffff';'\U10000':'\U1ffff'] if isprint(ch)]

rand_ascii(n) = randstring(asc_rng, n)
rand_latin(n) = trunc_str(lat_rng, n, rand(2:ceil(Int,n/1.45)))
rand_ucs2(n)  = trunc_str(ucs_rng, n, rand(2:div(n,3)))
rand_uni(n)   = trunc_str(uni_rng, n, rand(2:div(n,3)))

function generate_files(lines = 1000; dir::Any=nothing)
    sampledir = joinpath(getdefdir(dir), smppath)
    mkpath(sampledir)
    for maxlen = (7, 15, 255),
        (nam, fun) = (("ascii", rand_ascii), ("latin", rand_latin),
                      ("ucs2", rand_ucs2), ("uni", rand_uni))
        fnam = joinpath(sampledir, string(nam, "_", maxlen))
        println("Creating ", fnam)
        open(fnam, "w") do io
            for i = 1:lines
                println(io, fun(maxlen))
            end
        end
    end
end
...
function Base.show(io::IO, v::Tuple{String,CharStat})
    ...
    pr"\(io)\%10d(s.minlen)\%10d(s.maxlen)\n\n"
...
# "this is a test" is too long for ShortString7
searchstr(lines::Vector{String}, d=false) = searchlines(lines, "this is", d)
searchstr(lines::Vector{T}, d=false) where {T} = searchlines(lines, T("this is"), d)
...
function checktests(io = _stdout(); dir::Any=nothing, test::Bool=false)
    ...
    for fname in readdir(sampledir)
        ...
        list = [String, UTF8Str, UTF8String, ShortString255]
        maxlen = parse(Int, fname[findfirst(==('_'), fname)+1:end])
        maxlen <= 15 && push!(list, ShortString15)
        maxlen <= 7 && push!(list, ShortString7)
        println(list)
        println(fname)
